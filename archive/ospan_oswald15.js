...
/*
2022-11-23: Van Liceralde copied van Hedger (2020)'s reading span code and turned it into operation span
            Modified to be compatible with jsPsych v7.3; see the following migration guide for changes that had to be made
			https://www.jspsych.org/7.0/support/migration-v7/
			Modified instructions to be appropriate for operation span
			Added additional code based on James et al. MATLAB code and reference of WM tasks (Unsworth et al., 2005; 2009)
			- Added a function computing standard deviation because response deadline for participants is meant to be mean + 2.5 SD but only mean in original rspan.js
			- Made buttons during recall phase much larger, similar to what's seen in Unsworth et al. programs

/*
Overview:
This is a jsPsych implementation of the popular
automated operation span task, which is one of
several 'complex span' assessments of working
memory. Participants solve equations and determine
whether or not they make sense. This equation
...
equation/letter process anywhere from 3-7 times
on a trial.
Then, participants must recall the letter string
IN ORDER to the best of their ability. Participants
are also encouraged to keep their equation accuracy
above 85%, keeping in line with prior research.
To Use:
1. Load this js file in your html file.
2. Reference the css (ospanstyle.css) in your html file
3. Push the variable 'ospan_final' to your timeline in your html file
References:
...
var ospan_instruct_1 = {
	...
	stimulus: "<p style='font-size:25px'><b>Math and Memory Task</b></p><p style='font-size:25px'>In this task, you will try to memorize letters you see on the screen while you also solve simple math problems.</p>" +
	...
	on_finish: function(){practice = true; showEqnACC = false;} //switching practice to true for dynamic data, switching equation feedback to false
};

var ospan_instruct_2 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>For this practice set, letters will appear on the screen one at a time. Try to remember each letter in the order presented.</p>" +
...
var ospan_instruct_3 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>Remember, it is very important to get the letters in the same order as you see them.</p>" +
...
var ospan_instruct_4 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>Now you will practice doing the problem solving part of the task.</br>" +
...
var ospan_instruct_5 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>You will then see a possible answer to the math problem displayed on the next screen, like this:</p>" +
...
var ospan_instruct_6 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>It is VERY important that you answer the math problems correctly.</p>" +
...
var ospan_instruct_8 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>After the letter goes away, another math problem will appear, and then another letter.</p>" +
...
var ospan_instruct_9 = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px'>On the feedback screen, you will also see a number in the top right corner.</br>This indicates your percent correct for the math problems for the entire task.</p>" +
	...
	post_trial_gap: 250,
	button_html: '<button class="buttonStyle">%choice%</button>',
	on_finish: function(){showEqnACC = true;} //switching to show equation accuracy for letter+equation practice
};
...
var letter_presentation = {
	...
	stimulus: function(){return '<div style="font-size:75px; font-family: Arial; font-weight: bold;">' + correctSEQ[numIndex] + '</div>';},
...
var equation_presentation_practice = {
	...
	stimulus: '',
	post_trial_gap: 250,
	choices: [function(){ return '<p style="font-size: 45px;">' + practice_equations[calibRTindex].stimulus + '</p><br><br><p style="font-size: 15px;">When you have solved this math problem,</br>click on the screen to continue.</p>';}],
...
var equation_judgment_practice = {
	type: jsPsychHtmlButtonResponse,
	stimulus: function(){ return '<p style="font-size: 45px; font-family: Arial;">' + practice_equations[calibRTindex - 1].probe + '</p>'},
	choices: ["TRUE", "FALSE"],
	button_html: '<button class="buttonStyle">%choice%</button>',
	on_finish: function(data) {
...
var overall_practice_feedback = {
	...
	stimulus: function(){
		if(eqnPracticeCorrect > 12) {
			return '<p style="font-size: 25px;">You responded correctly on ' + eqnPracticeCorrect + ' of ' + calibRTindex + ' equations. Good job!</p>';
		} else {
			return '<p style="font-size: 25px;">You only responded correctly on ' + eqnPracticeCorrect + ' of ' + calibRTindex + ' equations.</p><p style="font-size: 20px;"><b>In the main task, you must try harder to answer accurately.</b></p>';}
		},
	choices: ['CONTINUE'],
	button_html: '<button class="buttonStyle">%choice%</button>'
};
...
var equation_judgment_practice_combined = {
	type: jsPsychHtmlButtonResponse,
	stimulus: function() { return '<p style="font-size: 45px;">' + main_equations[mainSelectionIndex - 1].probe + '</p>'},
	choices: ["TRUE","FALSE"],
	button_html: '<button class="buttonStyle">%choice%</button>',
	on_finish: function(data) {
		equationCRESP = jsPsych.data.get().last(2).values()[0].equationCRESP;
...
var equation_presentation_main = {
	...
	stimulus: '',
	post_trial_gap: 250,
	choices: [function(){ return '<p style="font-size: 45px;">' + main_equations[mainSelectionIndex].stimulus + '</p><br><br><p style="font-size: 15px;">When you have solved this math problem,</br>click on the screen to continue.</p>';}],
	button_html: '<button class="fullscreenStyle">%choice%</button>',
...
var equation_judgment_main = {
	type: jsPsychHtmlButtonResponse,
	stimulus: function() { return '<p style="font-size: 45px; font-family: Arial;">' + main_equations[mainSelectionIndex - 1].probe + '</p>'},
	choices: ["TRUE","FALSE"],
	button_html: '<button class="buttonStyle">%choice%</button>',
	on_finish: function(data) {
		equationRESP = data.response;
	...
	on_finish: function(data){
		...
		for (i = 0; i < correctSEQ.length; i++) {
			if (correctSEQ[i] == response[i]) {
				if(practice == false){OSPAN_TOTAL +=1;} //add to ospan total if not practice
				trialCorrect[i] = 1;
	...
	stimulus: function(){
		...
		if(showEqnACC==true){
			...
			if(currentACC > 84){var sentFont = "green"}else{var sentFont = "red"} //assign colored font based on 85% accuracy threshold
				...
				var equationTicker = '<p class = "senFB" id="senFB">Math Problems:</br>'+ currentACC.fontcolor(sentFont) +'</p>';
			} else {
				var equationTicker = '';
			}
		var getFeedback = jsPsych.data.get().last(1).values()[0].feedback;
		var feedbackText = getFeedback.join(" ");
		var pageText = '<p style="font-size: 60px; font-family:Arial; font-weight:bold;">' + feedbackText + '</p>' + equationTicker +
						'<p style="font-size:28px;"> You correctly identified ' + jsPsych.data.get().last(1).values()[0].numCorrect + ' of  '+jsPsych.data.get().last(1).values()[0].LENGTH+' letters.</p><br>';
...
var welcome = {
	type: jsPsychHtmlButtonResponse,
	stimulus: "<p style='font-size:25px;'><b>Math and Memory Task</b></p>" +
...
var practice_twoletter_trial = {
	timeline: [set_up_2, letter_presentation, letter_presentation, ospan_recall, feedback_screen]
	};


var practice_threeletter_trial = {
	timeline: [set_up_3, letter_presentation, letter_presentation, letter_presentation, ospan_recall, feedback_screen]
	};

//final letter practice proc; VL: E-Prime file only has two practice trials for set size 2
var letter_practice_final = {
	timeline: [letter_instructions, practice_twoletter_trial, practice_twoletter_trial]
};
...
var equation_practice = {
	timeline: [
	equation_presentation_practice, equation_judgment_practice, equation_judgment_feedback,
	equation_presentation_practice, equation_judgment_practice, equation_judgment_feedback,
	equation_presentation_practice, equation_judgment_practice, equation_judgment_feedback,
	equation_presentation_practice, equation_judgment_practice, equation_judgment_feedback,
	equation_presentation_practice, equation_judgment_practice, equation_judgment_feedback,
...
var equation_practice_final = {
	timeline: [equation_instructions, equation_practice, overall_practice_feedback]
};
...
var letterequation_practice = {
	timeline: [
	equation_presentation_main, if_timeout_practice, letter_presentation,
	equation_presentation_main, if_timeout_practice, letter_presentation
	]
};
...
var letterpractice_run = {
	timeline: [set_up_2, letterequation_practice, letterequation_practice_feedback],
};

//final combined practice proc
var letterequation_practice_final = {
	timeline: [letterequation_instructions, letterpractice_run, letterpractice_run, ospan_instruct_10]
	};
...
var ospan_3_core = {
	timeline: [
    equation_presentation_main, if_timeout_node, letter_presentation,
    equation_presentation_main, if_timeout_node, letter_presentation,
    equation_presentation_main, if_timeout_node, letter_presentation
]
}
...
var ospan_summary = {
    ...
    stimulus: function(){return "<p style='font-size:25px;'>There are two scores typically associated with this task.<br>" +
								...
								"If you correctly recalled 4 of 4 letters on a trial, both your TOTAL and ABSOLUTE score would increase by 4.</p>" +
									"<p style='font-size:25px;'></br></br>Your <b>TOTAL</b> score was " + OSPAN_TOTAL + ".</p><p 'font-size:25px;'>Your <b>ABSOLUTE</b> score was " + OSPAN_ABS +".</p>" +
...
//main ospan task
var ospan_final = {
	timeline: [get_participant_id, welcome, enter_fullscreen, 
		letter_practice_final, equation_practice_final, letterequation_practice_final,
		final_combined_runs, final_combined_runs, ospan_done, ospan_summary]
};

// Function to give the option for a local save of the data; doesn't work, needs exploring how to make work
// var localSave;

// function saveData() {
// 	if(localSave == 1) {
// 		var identifier = 'OSPAN_'+Math.round(new Date().getTime()/1000)+'.csv';
...
var conclusion = {
	...
	on_start: function(data) {
		if (online == 0) {
			var filename = "data_ospan_" + subject + ".csv";
